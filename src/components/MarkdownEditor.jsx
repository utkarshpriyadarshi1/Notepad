...
export default function MarkdownEditor({text, onUpdate}) {
    ...
    if (isEditing) {
        return (<div
                className="flex-1 flex flex-col overflow-hidden no-drag bg-white/20 p-2.5 rounded-xl border border-black/5 mb-2">
                <textarea
                    value={text}
    ...
    return (<div
            ...
            <div
                ...
                dangerouslySetInnerHTML={{__html: marked.parse(text || "*Empty note profile. Click here to initialize.*")}}
            />
        </div>);
}
